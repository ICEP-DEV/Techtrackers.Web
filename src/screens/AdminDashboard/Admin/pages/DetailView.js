  ...
  const renderActionButtons = () => {
    ...
    return (
      <div className={styles['action-buttons']}>
        <button className={styles['close-button']} onClick={() => handleOpenPopup('close')}>CLOSE</button>
        <button className={styles['reopen-button']} onClick={() => handleOpenPopup('reopen')}>RE-OPEN</button>
    ...
    <div className={styles['dashboard-container']}>
      <div className={styles['detail-header']}>
        ...
        <div className={styles['header-actions']}>
          <button className={styles['live-chat-button']} onClick={() => setIsChatOpen(true)}>
      ...
      <div className={styles['detail-content']}>
        {isEscalationVisible ? (
          ...
          <>
            {isConfirmed ? (
              <div className={styles['status-label']} style={{ color: actionType === 'close' ? 'green' : 'red' }}>
                <p>{statusMessage}</p>
                <h3>Issue Details:</h3>
                <div className={styles['issue-info']}>
                  ...
                  <div className={styles['issue-priority']}>
                    <p>Priority: <span className={`${styles['priority']} ${styles[log.priority.toLowerCase()]}`}>{log.priority}</span></p>
                  </div>
                ...
                <div className={styles['issue-status']}>
                  <p>Status: <span className={`${styles['status']} ${styles[log.status.toLowerCase()]}`}>{log.status}</span></p>
                </div>
                <button className={styles['back-button-d']} onClick={onBack}>BACK</button>
              </div>
            ) : (
                ...
                <div className={styles['issue-attachments']}>
                  ...
                  {log.attachments.length > 0 ? (
                    log.attachments.map((attachment) => (
                      <div className={styles['attachment']} key={attachment.filename} onClick={() => handleOpenImageViewer(attachment.imageUrl)}>
                        <img src={attachment.imageUrl} alt={attachment.filename} />
      ...
      {isPopupVisible && (
        <div className={styles['popup-overlay']}>
          <div className={styles['popup-content']}>
            <p>{actionType === 'close' ? 'Are you sure you want to close this log?' : 'Are you sure you want to reopen this log?'}</p>
      ...
      {/* Image Viewer Popup */}
      {isImageViewerOpen && (
        <div className={styles['image-viewer-overlay']} onClick={handleCloseImageViewer}>
          <div className={styles['image-viewer-content']} onClick={(e) => e.stopPropagation()}>
            <img src={selectedImage} alt="Attachment" className={styles['viewed-image']} />
            <button className={styles['close-image-button']} onClick={handleCloseImageViewer}>Close</button>
          </div>
        </div>
      )}
    </div>
