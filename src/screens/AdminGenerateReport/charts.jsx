...
const IssueReport = () => {
  ...
  const exportToExcel = () => {
    const worksheetData = [
      ...tableData,
      {}, // Empty row
      { status: 'Status', issues: 'Number of Issues' },
      { status: 'OPEN', issues: 30 },
      { status: 'IN PROGRESS', issues: 20 },
      { status: 'COMPLETED', issues: 50 },
      {}, // Empty row
      { status: 'Distribution', percentage: 'Percentage' },
      { status: 'COMPLETED', percentage: '47%' },
      { status: 'OPEN', percentage: '33%' },
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-around', marginTop: '40px' }}>
        <div style={{ width: '45%' }}>
          ...
          <Bar ref={barChartRef} data={barChartData} options={{ responsive: true }} />
        </div>
        <div style={{ width: '45%' }}>
          <h3 style={{ textAlign: 'center' }}>ISSUE STATUS DISTRIBUTION</h3>
          <Pie ref={pieChartRef} data={pieChartData} options={{ responsive: true }} />
        </div>
      </div>
