        ...
        <main className="issue-details-container">
          ...
          <div className="issue-info">
            <div className="info-group">
              {issue.department && <p>Department - {issue.department}</p>}
              <p>Building 18 - 2nd Floor</p>
            </div>
            <div className="info-group">
              {issue.priority && (
                <p>
                  Priority: <span className={`priority ${issue.priority.toLowerCase()}`}>{issue.priority}</span>
                </p>
              )}
            </div>
            <div className="info-group">
              <button className="live-chat-button" onClick={onOpenChat}>Live Chat</button>
              {issue.date && <p>{issue.date}</p>}
            </div>
          </div>
